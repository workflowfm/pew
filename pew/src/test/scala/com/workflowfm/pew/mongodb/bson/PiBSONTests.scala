...
//  }
  ...
  "ChanMapCodec" should "encode/decode ChanMaps" in {
    ...
    roundTrip(
      ChanMap(
        (Chan("X"), PiItem("Oh!")),
        (Chan("Y"), PiPair(Chan("L"), PiItem("R"))),
        (Chan("2"), PiOpt(Chan("L"), PiPair(Chan("RL"), Chan("RR"))))
      ),
      codec
    )
  }
  ...
  "PiStateCodec" should "encode/decode PiStates" in {
    ...
    roundTrip(
      PiState(
        ...
        Out("A", Chan("X"))
      ) withProc proc1 withCalls proc1.getFuture(0, Chan("X"), Chan("R")) withSub ("INPUT", PiItem(
            "OHHAI!"
          )),
      codec
    )
    roundTrip(
      ...
      ) withProc proc1 withThread (0, "PROC", "R", Seq(
      ...
      PiInstance(
        ...
        PiState(
          ...
          Out("A", Chan("X"))
              ...
              .getFuture(0, Chan("X"), Chan("R")) withSub ("INPUT", PiItem("OHHAI!"))
      ),
      codec
    )
    roundTrip(
...
trait PiBSONTestHelper {
  ...
  def roundTripCodec[T](value: T, codec: Codec[T]): Unit = {
    ...
    System.out.println(
      s"Round Tripped case class: ($roundTripped) must equal the original: ($value)"
    )
    assert(
      roundTripped == value,
      s"Round Tripped case class: ($roundTripped) did not equal the original: ($value)"
    )
  }
  ...
  def roundTripCodec[T](value: T, expected: Document, codec: Codec[T]): Unit = {
    ...
    assert(
      expected == actual,
      s"Encoded document: (${actual.toJson()}) did not equal: (${expected.toJson()})"
    )
  ...
  def documentOf[T](codec: Codec[T], value: T): Document =
    decode(documentCodec, encode(codec, value))
  ...
  def decode[T](codec: Codec[T], buffer: OutputBuffer): T = {
    val reader = new BsonBinaryReader(
