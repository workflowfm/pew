...
object SelectSkiInst extends SelectSki { 
  override def apply( arg0 : LengthInch, arg1: Brand, arg2: Model ): Future[Either[PriceUSD,Exception]] = {
		println("SelectSki received: "+arg0 + " - " + arg1 + " - " + arg2)
    if (arg0.length() > 23) Future.successful(Left("PriceUSD("+arg0+","+arg1+","+arg2+")"))
...
class R1(pa : Pa,pb : Pb) extends CompositeProcess { // (X) => Y
	...
	override val body = PiCut("z2","cPb_A_1","oPa_A_",PiCall<("Pb","cPb_A_1","oPb_Y_"),PiCall<("Pa","cPa_X_1","oPa_A_"))
}
...
class R2(pc : Pc,r1 : R1) extends CompositeProcess { // (X) => Z
	...
	override val body = PiCut("z2","cPc_Y_1","oPb_Y_",PiCall<("Pc","cPc_Y_1","oPc_Z_"),PiCall<("R1","cPa_X_1","oPb_Y_"))
}

trait Pd extends ((X) => Either[A,(B,G)]) with AtomicProcess {
	override val name = "Pd"
	override val output = (PiOpt(Chan("Pd_l_a_A"),PiPair(Chan("Pd_rl_a_B"),Chan("Pd_rr_a_G"))),"oPd_lB_A_Plus_lB_B_x_G_rB_rB_")
...
class R3(pd : Pd,pe : Pe) extends CompositeProcess { // (X) => Either[A,(Y,G)]
	...
	override val output = (PiOpt(Chan("R3_l_a_A"),PiPair(Chan("R3_rl_a_Y"),Chan("R3_rr_a_G"))),"y13")
	override val inputs = Seq((Chan("R3__a_X"),"cPd_X_1"))
	override val channels = Seq("cPd_X_1","y13")

	override val dependencies = Seq(pd,pe)

	override val body = PiCut("z15","x13","oPd_lB_A_Plus_lB_B_x_G_rB_rB_",WithIn("x13","a13","z12",LeftOut("y13","b13",PiId("a13","b13","m14")),RightOut("y13","z10",ParInI("z12","b5","buf10",ParOut("z10","oPe_Y_","b10",PiCut("z5","cPe_lB_B_Plus_A_rB_1","cPe_lB_B_Plus_A_rB_1",PiCall<("Pe","cPe_lB_B_Plus_A_rB_1","oPe_Y_"),LeftOut("cPe_lB_B_Plus_A_rB_1","x6",PiId("b5","x6","m7"))),PiId("buf10","b10","m11"))))),PiCall<("Pd","cPd_X_1","oPd_lB_A_Plus_lB_B_x_G_rB_rB_"))
}

class R3b(pd : Pd,pe : Pe) extends CompositeProcess { // (X) => Either[Y,(B,G)]
	override val name = "R3b"
	override val output = (PiOpt(Chan("R3b_l_a_Y"),PiPair(Chan("R3b_rl_a_B"),Chan("R3b_rr_a_G"))),"y6")
	override val inputs = Seq((Chan("R3b__a_X"),"cPd_X_1"))
	override val channels = Seq("cPd_X_1","y6")

	override val dependencies = Seq(pd,pe)

	override val body = PiCut("z11","x6","oPd_lB_A_Plus_lB_B_x_G_rB_rB_",WithIn("x6","b3","c6",LeftOut("y6","oPe_Y_",PiCut("z3","cPe_lB_B_Plus_A_rB_1","cPe_lB_B_Plus_A_rB_1",PiCall<("Pe","cPe_lB_B_Plus_A_rB_1","oPe_Y_"),RightOut("cPe_lB_B_Plus_A_rB_1","y4",PiId("b3","y4","m5")))),RightOut("y6","d6",ParInI("c6","x7","y7",ParOut("d6","x8","y8",PiId("x7","x8","m9"),PiId("y7","y8","m10"))))),PiCall<("Pd","cPd_X_1","oPd_lB_A_Plus_lB_B_x_G_rB_rB_"))
}
...
trait SelectLength extends ((HeightCM, WeightKG) => Future[LengthCM]) with AtomicProcess {
	...
	override val inputs = Seq((Chan("SelectLength__a_HeightCM"),"cSelectLength_HeightCM_1"),(Chan("SelectLength__a_WeightKG"),"cSelectLength_WeightKG_2"))
	override val channels = Seq("cSelectLength_HeightCM_1","cSelectLength_WeightKG_2","oSelectLength_LengthCM_")

	def run(args:Seq[PiObject])(implicit ec:ExecutionContext):Future[PiObject] = args match {
		case Seq(o1,o2) => this(PiObject.getAs[HeightCM](o1),PiObject.getAs[WeightKG](o2)) map PiObject.apply
	}
}
...
class GetSki(cM2Inch : CM2Inch,selectLength : SelectLength,selectModel : SelectModel,selectSki : SelectSki,uSD2NOK : USD2NOK) extends CompositeProcess { // (HeightCM, PriceLimit, SkillLevel, WeightKG) => Either[PriceNOK,Exception]
	...
	def apply(heightCM : HeightCM,priceLimit : PriceLimit,skillLevel : SkillLevel,weightKG : WeightKG)(implicit executor:FutureExecutor): Future[Option[Either[PriceNOK,Exception]]] =
