...
class GetSki(
    ...
    selectSki: SelectSki,
    uSD2NOK: USD2NOK
) extends CompositeProcess { // (HeightCM, PriceLimit, SkillLevel, WeightKG) => Future[Either[PriceNOK,Exception]]
  ...
  def apply(heightCM: HeightCM, priceLimit: PriceLimit, skillLevel: SkillLevel, weightKG: WeightKG)(
      implicit executor: ProcessExecutor[_]
  ): Future[Either[PriceNOK, Exception]] = {
