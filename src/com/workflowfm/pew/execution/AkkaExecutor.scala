...
class AkkaExecutor(store:PiInstanceStore[Int], processes:Map[String,PiProcess])(implicit system: ActorSystem, override implicit val context: ExecutionContext = ExecutionContext.global, implicit val timeout:FiniteDuration) extends FutureExecutor {
  ...
  def this(l:PiProcess*)(implicit system: ActorSystem, context: ExecutionContext, timeout:FiniteDuration) = this(SimpleInstanceStore(),PiProcess.mapOf(l :_*))
...
object AkkaExecutor {
  ...
  case class ACall(id:Int,ref:Int,p:AtomicProcess,args:Seq[PiObject],act:ActorRef)
