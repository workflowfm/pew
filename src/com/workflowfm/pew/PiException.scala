...
object PiException {
  implicit def apply[KeyT]: PiException[KeyT] => PiExceptionEvent[KeyT] = {
    ...
    case ex: RemoteException[KeyT]                    => PiEventException( ex.id, ex.message, ex.stackTrace, System.currentTimeMillis() )
    case ex: RemoteProcessException[KeyT]             => PiEventProcessException( ex.id, ex.ref, ex.message, ex.stackTrace, System.currentTimeMillis() )
  }
}
