package com.workflowfm.pew.kafka.components
...
abstract class Tracked[Value] {
  ...
  /** Apply a fold to 2 messages so that their tracking information can be combined.
    ...
    */
  protected def fold[NewValue](fn: (Value, NewValue) => Value)(
      other: Tracked[NewValue]
  ): Tracked[Value]
}
...
object Tracked {
  ...
  /** Expose a future contained within a Tracked wrapper,
    ...
    */
  def exposeFuture[V, T[X] <: Tracked[X]](fut: T[Future[V]])(
      implicit exec: ExecutionContext
  ): Future[T[V]] = fut.value.map(freplace(fut))(exec)
...
case class CommitTracked[Value](
  ...
  override protected def fold[NewValue](
      fn: (Value, NewValue) => Value
  )(other: Tracked[NewValue]): Tracked[Value] = copy(
...
object CommitTracked extends TrackedSource[CommitTracked] with HasCommittableSinks[CommitTracked] {
  ...
  ): Source[CommitTracked[Value], Control] = Consumer
...
object PartTracked extends TrackedSource[PartTracked] with HasCommittableSinks[PartTracked] {
  ...
  def source[Value](
      cs: ConsumerSettings[_, Value],
      sub: AutoSubscription
  ): Source[PartTracked[Value], Control] = Consumer
...
case class Transaction[Value](
  ...
  override protected def fold[NewValue](
      fn: (Value, NewValue) => Value
  )(other: Tracked[NewValue]): Tracked[Value] = {
    assert(
      assertion = false,
      "This function should work, but causes hangs when used with Transactional.sink"
    )
    ...
    require(
      partOffset.key == that.partOffset.key,
      "Trying to merge offsets of different partitions."
    )
...
object Transaction
  ...
  override def source[Value](
      cs: ConsumerSettings[_, Value],
      sub: AutoSubscription
  ): Source[Transaction[Value], Control] = Transactional
    .source(cs, sub)
    .map(msg => Transaction(msg.record.value(), msg.partitionOffset))

  override def sink[V <: AnyMsg](
      implicit s: KafkaExecutorEnvironment
  ): Sink[Transaction[V], Future[Done]] = {
  ...
  override def sinkMulti[V <: AnyMsg](
      implicit s: KafkaExecutorEnvironment
  ): Sink[Transaction[Seq[V]], Future[Done]] = {
    // TODO: Construct the KafkaProducer with the correct ClassLoader
    val id = ObjectId.get.toString
    Transactional
      .sink(s.psAllMessages, id)
...
case class Untracked[Value](
  ...
  override protected def fold[NewValue](fn: (Value, NewValue) => Value)(
      other: Tracked[NewValue]
  ): Tracked[Value] = copy(value = fn(value, other.value))
}
...
object Untracked extends TrackedSource[Untracked] with TrackedSink[Untracked] {
  ...
  override def source[Value](
      cs: ConsumerSettings[_, Value],
      sub: AutoSubscription
  ): Source[Untracked[Value], Control] = Consumer
    .committableSource(cs, sub)
    .mapAsync(1)(message =>
...
case class MockTransaction[Value](
  ...
  override protected def fold[NewValue](
    ...
    require(
      that.minOffset == maxOffset,
      s"Messages cannot be skipped! ($maxOffset -> ${that.minOffset})"
    )
...
object MockTransaction {
  ...
  def source[Value](messages: Seq[(Value, Int)]): Source[MockTransaction[Value], NotUsed] = Source
...
object MockTracked {
  ...
  def source[Value](messages: Seq[(Value, Int)]): Source[MockTracked[Value], NotUsed] = Source
    .fromIterator(() => messages.iterator)
