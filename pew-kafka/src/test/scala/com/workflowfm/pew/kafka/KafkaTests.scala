...
trait KafkaTests extends ProcessExecutorTester {
  ...
  lazy val completeProcess: ProcessType = new ProcessType(
    SimpleProcessStore(
      pai,
      pbi,
      pci,
  ...
  lazy val failureProcess: ProcessType = new ProcessType(
    SimpleProcessStore(
      pai,
      pbi,
      pcif,
  ...
  lazy val shutdownProcess: ProcessType = new ProcessType(
    SimpleProcessStore(
      pai,
      pbi,
      pciw,
  ...
  private def outstanding(consume: Boolean): Seq[AnyMsg] = {
    ...
    val fOutstanding: Future[Seq[AnyMsg]] = KafkaWrapperFlows.srcAll
      .wireTap(msg =>
  ...
  class MessageDrain(consume: Boolean = false) extends MessageMap(outstanding(consume))

  def toLogMap(msgMap: MessageMap): LogMap = new ClassMap[PiEvent[_]](msgMap[PiiLog] map (_.event))
