package com.workflowfm.pew.kafka.component
...
class KafkaAtomicExecTests extends PewTestSuite with KafkaExampleTypes {
  ...
  val assgnException: Assignment = Assignment(
  ...
  val assgnCompositeProc: Assignment = Assignment(
  ...
  val assgnUnknownProc: Assignment = Assignment(
    eg1.pInProgress,
    eg1.r1._1,
    ":`(",
    Seq(PiResource(eg1.arg1, pai.inputs.head._1))
  ...
  it should "respond to Assignments with a correct SequenceRequest" in {
    ...
    val (threads, pii) =
      PiInstance(ObjectId.get, pbi, PiObject(1)).reduce.handleThreads((_, _) => true)
  ...
  it should "correctly handle a batch of Assignments on different partitions" in {
    val msgsOf =
      runAEx(
        ...
        (eg1.assgnFinishing2, 2),
        (eg1.assgnFinishing3, 3),
        (eg1.assgnInProgress, 2),
        (eg1.assgnFinishing2, 3),
        (eg1.assgnFinishing3, 1),
        (eg1.assgnInProgress, 3),
        (eg1.assgnFinishing2, 1),
        (eg1.assgnFinishing3, 2),
        (eg1.assgnInProgress, 1),
        (eg1.assgnFinishing2, 2),
        (eg1.assgnFinishing3, 3)
      )
